...
class ImageWebScrapper:
    def __init__(self, image_directory, image_limit: int = 10):
        self.image_directory = image_directory
        self.website_headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko)'}
...
#       calling necessary classes
    ...
    def get_files(self):
        louis_vuiton_dir = os.path.join(self.image_directory, "louisvuitton.com")
        image_paths = list()
        print(self.image_file_manager.get_files_list(louis_vuiton_dir))
        for file_name in self.image_file_manager.get_files_list(louis_vuiton_dir):
            full_path_image = os.path.join(louis_vuiton_dir, file_name)
    ...
    def analyze_image(self, file_path):
        ...
        print(len(most_common_colors))
        
        self.image_analysis.plot_rgb_scatter_with_frequency(most_common_colors, file_path)
    
    def analyze_all_images(self):
        louis_vuiton_dir = os.path.join(self.image_directory, "louisvuitton.com")
        files = self.image_file_manager.get_files_list(louis_vuiton_dir)
        
        all_images_rgb_count = list()
        for file in files:
            file_path = os.path.join(louis_vuiton_dir, file)
            image = self.image_analysis.file_to_image(file_path)
            colors = self.image_analysis.image_main_colors(image)
            most_common_colors = self.image_analysis.image_most_common_colors(colors)
            all_images_rgb_count.append(most_common_colors)
        
        sorted_top_values  = sorted(self.image_analysis.get_top_values(all_images_rgb_count))
        print("\n\n1: ", sorted_top_values, "\n\n")
        print("\n\n2: ", most_common_colors, "\n\n")
        
        self.image_analysis.plot_rgb_scatter_with_frequency(self.image_analysis.image_most_common_colors(sorted_top_values), file_path)
...
#     facade.analyze_image(r'C:\Users\JJ\Documents\jupyterNotebookProjects\python\jupyter\images\louisvuitton.com\0.png')
